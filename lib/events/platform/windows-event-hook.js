...
const log = createModuleLogger("windows-event-hook");

export class WindowsEventHook extends BaseEventAdapter {
  constructor() {
    super();
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (window) {
          ...
          if (app !== this._lastApp || title !== this._lastTitle) {
            this._lastApp = app;
            this._lastTitle = title;
            this._emitNormalized("app:switch", { app, title, prevApp: this._lastApp });
          }
        }
      } catch (err) {
  ...
  async _startNative() {
    log.warn("native hook not yet implemented, using polling fallback");
    this._startPollingFallback();
  }
  ...
  _simulateNativeEvent(nativeType, data) {
    switch (nativeType) {
      case "foreground": {
        ...
        } else if (title !== this._lastTitle) {
          this._lastTitle = title;
          this._emitNormalized("window:focus", { app, title });
        }
        break;
      }
      case "mouse":
