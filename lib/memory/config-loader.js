...
function _watchConfigFile(configPath, callback) {
  if (_watchers.has(configPath)) return;
  ...
  try {
    ...
    const watcher = fs.watch(dir, (eventType, filename) => {
      if (filename === basename && (eventType === "change" || eventType === "rename")) {
    ...
    watcher.unref?.();
    _watchers.set(configPath, watcher);
  } catch (err) {
    // 文件监听在某些环境下不可用,静默失败
  }
}

export function loadConfigWithWatch(configPath, callback) {
  const config = loadConfig(configPath);
  _watchConfigFile(configPath, callback);
  return config;
}

export function stopWatching(configPath) {
  const watcher = _watchers.get(configPath);
  if (watcher) {
    watcher.close();
    _watchers.delete(configPath);
  }
}
