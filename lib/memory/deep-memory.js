...
// ── 事实自动分类：检测 feedback / preference 类型 ──
...
const PREFERENCE_LABELS = new Set([
  "preference", "喜欢", "不喜欢", "兴趣", "审美", "偏好",
  "food", "music", "movie", "book", "sport", "hobby",
]);

/**
 * 自动检测事实类型，返回类型和需追加的标签
 * @param {string} fact - 事实文本
 * @param {string[]} existingTags - LLM 已有的标签
 * @returns {{ type: 'fact'|'feedback'|'preference', extraTags: string[] }}
 */
function classifyFact(fact, existingTags) {
  const extraTags = [];
  let type = "fact";

  // 检测 feedback 类型：包含纠正信号
  const isFeedback = FEEDBACK_PATTERNS.some((p) => p.test(fact));
  if (isFeedback) {
    type = "feedback";
    if (!existingTags.includes("feedback")) extraTags.push("feedback");
  }

  // 检测 preference 类型：LLM 标签已包含偏好相关标签
  const hasPreferenceTag = existingTags.some((t) => PREFERENCE_LABELS.has(t));
  if (hasPreferenceTag) {
    if (type === "fact") type = "preference";
    if (!existingTags.includes("preference")) extraTags.push("preference");
  }

  return { type, extraTags };
}
...
export async function processDirtySessions(summaryManager, factStore, resolvedModel, opts = {}) {
  ...
  const detector = new ContradictionDetector(factStore, resolvedModel);
  let totalFacts = 0;
  ...
  const processOne = async (session) => {
    try {
      ...
      if (facts.length > 0) {
        // 自动分类：为事实追加 feedback / preference 标签和 type 字段
        const classifiedFacts = facts.map((f) => {
          const { type, extraTags } = classifyFact(f.fact, f.tags || []);
          return {
            fact: f.fact,
            type,
            tags: extraTags.length > 0 ? [...(f.tags || []), ...extraTags] : (f.tags || []),
            time: f.time || null,
            effective_time: f.effective_time || f.time || null,
        ...
        // 矛盾检测
        const factsToStore = [];
        for (const f of classifiedFacts) {
          const { hasContradiction, conflicts } = await detector.detect(f.fact, f.tags);

          if (hasContradiction) {
            log.log(`检测到矛盾: ${f.fact}`);

            // 标记旧事实为过时
            for (const conflict of conflicts) {
              const existingFact = factStore.getFactByText(conflict.existingFact);
              if (existingFact) {
                factStore.markAsOutdated(existingFact.id, `与新事实矛盾: ${f.fact}`);
                log.log(`标记旧事实为过时: ${conflict.existingFact}`);
              }
            }
          }
        ...
        factStore.addBatchWithEmbeddings(factsToStore)
          .then(({ count, embedded }) => {
            totalFacts += count;
            log.log(
              `${session.session_id.slice(0, 8)}...: ${count} 条元事实${embedded > 0 ? `, ${embedded} embeddings` : ""}`,
            );
          })
      }
...
[
  {"fact": "用户最近在关注记忆系统", "tags": ["记忆系统", "近况"], "time": null, "effective_time": null},
  {"fact": "用户希望长期记忆更像用户画像，而不是协作手册", "tags": ["用户画像", "长期记忆", "边界"], "time": null, "effective_time": null}
]

## effective_time 字段说明

- effective_time：事实开始生效的时间（ISO 8601 或自然语言）
- 如果是偏好/状态类事实，提取用户开始有此偏好的时间
- 如果是事件类事实，与 time 相同
- 如果无法确定，填 null`;
  }
...
[
  {"fact": "The user has recently been focused on memory systems", "tags": ["memory-systems", "current-state"], "time": null, "effective_time": null},
  {"fact": "The user wants long-term memory to behave more like a user profile than a collaboration manual", "tags": ["user-profile", "long-term-memory", "boundary"], "time": null, "effective_time": null}
]

## effective_time field description

- effective_time: the time when the fact became effective (ISO 8601 or natural language)
- For preference/state facts, extract the time when the user started having this preference
- For event facts, use the same value as time
- If unknown, use null`;
}
