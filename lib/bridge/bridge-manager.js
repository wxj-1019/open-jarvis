...
import { PriorityMessageQueue, getDelayByPriority, isValidPriority } from "./priority-queue.js";
...
export class BridgeManager {
  ...
  constructor({ engine, hub }) {
    ...
    this._rcMirrorUnsubscribe = typeof hub?.subscribe === "function"
      ...
      : null;
    this._infoQueue = new PriorityMessageQueue({ maxSize: 100 });
  }
   ...
   * 用于心跳/cron 升级到 IM 的场景，支持通知优先级分级。
   *
   * @param {string} text - 要发送的文本（会自动 clean mood/pulse 标签）
   * @param {string} [targetAgentId]
   * @param {{ priority?: "urgent"|"normal"|"info", contextPolicy?: "none"|"record_when_delivered", bridgePlatforms?: string[] }} [opts]
   * @returns {{ platform: string, chatId: string, sessionKey: string, recorded: boolean } | { status: "queued" } | null} 发送成功返回平台信息，info 级别返回 queued，失败返回 null
   */
  async sendProactive(text, targetAgentId, opts = {}) {
    const priority = isValidPriority(opts.priority) ? opts.priority : "normal";
    const cleaned = this._cleanReplyForPlatform(text);
    if (!cleaned) return null;

    if (priority === "info") {
      this._infoQueue.push({ text: cleaned, title: "", timestamp: Date.now() });
      debugLog()?.log("bridge", `→ info notification queued (queue size: ${this._infoQueue.size})`);
      return { status: "queued" };
    }

    const pendingInfo = this._infoQueue.clear();
    const combinedText = pendingInfo.length > 0
      ? `${cleaned}\n\n---\n${pendingInfo.map(m => m.text).join("\n")}`
      : cleaned;

    const delay = getDelayByPriority(priority);
    if (delay > 0) {
    ...
    for (const entry of deliveryEntries) {
      ...
      const spec = ADAPTER_REGISTRY[platform];
      try {
        await entry.adapter.sendReply(chatId, combinedText);
        debugLog()?.log("bridge", `→ ${platform} proactive to owner (${combinedText.length} chars, priority: ${priority})`);
