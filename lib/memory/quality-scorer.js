...
export function scoreConsistency(fact, allFacts) {
  ...
  for (const other of allFacts) {
    ...
    const sharesTag = factTags.some((t) => otherTags.includes(t));
  ...
  const conflictRatio = conflictCount / relatedCount;
...
export function createQualityScorer(config) {
  ...
  function score(fact, allFacts) {
    ...
    const consistency = scoreConsistency(fact, allFacts || []);
