...
import { createCompileRetryManager, createCircuitBreaker } from "./compile-retry.js";

const log = createModuleLogger("compile");
...
// ── 编译结果缓存：文件持久化，在 LLM 全部失败时降级到 24h 内的成功结果 ──
const CACHE_EXPIRY_MS = 24 * 60 * 60 * 1000;
const compileCache = new Map(); // stepName → { timestamp, result }

async function loadCompileCache(stepName) {
  return compileCache.get(stepName) || null;
}

async function saveCompileCache(stepName, result) {
  compileCache.set(stepName, { timestamp: Date.now(), result });
  // 定期清理过期缓存
  const cutoff = Date.now() - CACHE_EXPIRY_MS;
  for (const [k, v] of compileCache) {
    if (v.timestamp < cutoff) compileCache.delete(k);
  }
}

const retryManager = createCompileRetryManager({
  logger: {
    info: (msg) => log.log(msg),
    warn: (msg) => log.warn(msg),
    error: (msg) => log.error(msg),
  ...
  circuitBreaker,
  loadCache: loadCompileCache,
  saveCache: saveCompileCache,
});
...
export async function compileToday(summaryManager, outputPath, resolvedModel, opts = {}) {
  ...
  // ── 分级指纹：按 session_id 分组 ──
  const newFingerprints = {};
  for (const s of sessions) {
    newFingerprints[s.session_id] = computeFingerprint([s.summary, s.updated_at]);
  }

  let oldFingerprints = {};
  try {
    oldFingerprints = JSON.parse(fs.readFileSync(fpPath, "utf-8"));
  } catch {}

  const oldKeys = new Set(Object.keys(oldFingerprints));
  const newKeys = new Set(Object.keys(newFingerprints));
  const added = [...newKeys].filter(k => !oldKeys.has(k));
  const removed = [...oldKeys].filter(k => !newKeys.has(k));
  const changed = [...newKeys].filter(k => oldKeys.has(k) && oldFingerprints[k] !== newFingerprints[k]);
  const hasChanges = added.length > 0 || removed.length > 0 || changed.length > 0;

  if (!hasChanges && fs.existsSync(outputPath)) {
    fs.writeFileSync(fpPath, JSON.stringify(newFingerprints));
    return "skipped";
  }
...
export async function compileWeek(summaryManager, outputPath, resolvedModel, opts = {}) {
  ...
  // ── 分级指纹：按 session_id 分组，只在新增/变更 session 时才重编译 ──
  const newFingerprints = {};
  for (const s of sessions) {
    newFingerprints[s.session_id] = computeFingerprint([s.summary, s.updated_at]);
  }
  ...
  if (hasChanges) {
    debugLog()?.log("memory",
      `compileWeek fingerprints: ${added.length} added, ${removed.length} removed, ${changed.length} changed`);
  }
...
Output 3-5 weekly themes/events. Max 240 words. Do not output Markdown headings. Do not start with #, ##, or ###; output body text only.`,
    ...
    {
      fallbackPrompt: isZh
        ? "请用 3-5 句话概括过去 7 天用户主要关注了什么。只写大主题，不写细节。"
        : "Summarize in 3-5 sentences what the user was mainly focused on in the past 7 days. Only broad themes, no details.",
    },
  );
...
export async function compileFacts(summaryManager, outputPath, resolvedModel, opts = {}) {
  ...
  if (lines.length <= CHUNK_SIZE) {
    result = await _compileFactsSafely(
      combined,
      _getFactsPrompt(),
      resolvedModel,
      300,
    ...
    log.log(`compileFacts: ${lines.length} facts, compiling in ${chunks.length} chunks`);

    const chunkResults = [];
    for (const chunk of chunks) {
      const chunkResult = await _compileFactsSafely(
        chunk,
        _getFactsPrompt(),
        resolvedModel,
        300,
      );
      if (chunkResult) chunkResults.push(chunkResult);
    }
    ...
    if (chunkResults.length === 1) {
      ...
      result = await _compileFactsSafely(
        chunkResults.join("\n"),
        _isZh()
          ? "将以下已去重的重要事实再次合并去重（200字以内）。只保留稳定的用户画像信息。"
          : "Deduplicate and merge the following key facts again (under 120 words). Keep only stable user-profile facts.",
        resolvedModel,
        300,
      );
    }
  ...
  // LLM 全部失败时 result 为空，保留原始内容避免数据丢失
 ...
 * @param {string} stepName - 编译步骤名称（用于日志）
 * @param {object} [opts] - 可选参数
 * @param {string} [opts.fallbackPrompt] - 降级 prompt（主 prompt 失败时使用）
 */
async function _compactLLM(input, systemPrompt, resolvedModel, maxTokens, stepName, opts = {}) {
  ...
  const execOpts = { degrade: true };  // 启用编译缓存降级

  if (opts.fallbackPrompt) {
    const fallbackPrompt = opts.fallbackPrompt;
    execOpts.fallbackFn = async () => callLLM(fallbackPrompt);
  }

  return retryManager.executeWithRetry(() => callLLM(systemPrompt), stepName || "compile", execOpts);
}

// ── compileFacts 分块处理：防止 30 天摘要拼接后超出 LLM 上下文窗口 ──
const MAX_FACTS_CHARS = 6000; // 每块约 1500 tokens，留足余量给 system prompt
const MAX_MERGE_ITERATIONS = 5; // 防止异常输入导致无限循环

/**
 * 安全地执行 facts 去重合并：内容较小时直接调用 LLM，
 * 超出阈值时自动分块递归合并，避免上下文溢出。
 */
async function _compileFactsSafely(combined, systemPrompt, resolvedModel, maxTokens) {
  if (combined.length <= MAX_FACTS_CHARS) {
    return _compactLLM(combined, systemPrompt, resolvedModel, maxTokens, "compileFacts");
  }

  const compileLog = createModuleLogger("compile-facts-chunk");
  compileLog.log(`compileFacts: 文本过长 (${combined.length} 字)，分块处理...`);
  ...
  for (const s of sentences) {
    if (current.length + s.length > MAX_FACTS_CHARS && current.length > 0) {
  ...
  let results = [];
  for (const chunk of chunks) {
    const result = await _compactLLM(chunk, systemPrompt, resolvedModel, maxTokens, "compileFacts-chunk");
    if (result) results.push(result);
  }

  // 递归合并：两两合并直到只剩一条，或无法继续合并
  let iterations = 0;
  while (results.length > 1 && iterations < MAX_MERGE_ITERATIONS) {
    iterations++;
    const merged = [];
    ...
    for (let i = 0; i < results.length; i += 2) {
      ...
      const pair = right ? `${results[i]}\n${right}` : results[i];
      if (pair.length <= MAX_FACTS_CHARS && right) {
        try {
          const result = await _compactLLM(pair, systemPrompt, resolvedModel, maxTokens, "compileFacts-merge");
          ...
          // 单块合并失败时保留原块继续
          compileLog.warn(`合并失败 (${results[i].length}+${right.length} 字): ${err.message}`);
          merged.push(results[i]);
          merged.push(right);
          mergeFailed = true;
        }
        ...
        // 合并后超长 或 只剩单独一项 → 保留原样
        merged.push(results[i]);
        if (right) merged.push(results[i + 1]);
      }
    }

    // 无法继续压缩时退出（合并结果数量未减少）
    if (merged.length >= results.length) break;
    results = merged;

    // 合并过程中有失败，不再继续压缩
    if (mergeFailed) break;
  }

  // 最终结果：多条时用换行直接拼接（不再调 LLM 合并，避免无限循环）
