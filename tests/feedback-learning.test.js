...
describe("Schema Migration v4→v5 (type column)", () => {
  ...
  it("backfills type from existing feedback/preference tags", () => {
    ...
    db.prepare("INSERT INTO facts (fact, tags, created_at) VALUES (?, ?, ?)").run(
      "用户说不要使用 tab 缩进", JSON.stringify(["feedback", "coding-style"]), "2026-05-01T10:00:00.000Z"
    );
    db.prepare("INSERT INTO facts (fact, tags, created_at) VALUES (?, ?, ?)").run(
      "用户喜欢深色主题", JSON.stringify(["preference", "ui"]), "2026-05-02T10:00:00.000Z"
    );
    db.prepare("INSERT INTO facts (fact, tags, created_at) VALUES (?, ?, ?)").run(
      "普通事实不带类型标签", JSON.stringify(["general"]), "2026-05-03T10:00:00.000Z"
    );
    db.close();
    ...
    expect(normalFacts[0].fact).toContain("普通事实");
    store.close();
  });
});

// ═══════════════════════════════════════════
// getByType / getActiveFeedback 检索测试
// ═══════════════════════════════════════════
