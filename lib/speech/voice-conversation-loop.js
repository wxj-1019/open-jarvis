...
export class VoiceConversationLoop extends EventEmitter {
   ...
   * @param {import('./whisper-stt-adapter.js').WhisperSTTAdapter} [deps.whisperSTTAdapter]
  ...
  constructor(deps, opts = {}) {
    ...
    /** @type {import('./whisper-stt-adapter.js').WhisperSTTAdapter|null} */
    this._whisperSTT = deps.whisperSTTAdapter || null;
  ...
  _setupListeners() {
    this._onVadSpeechEnd = async (audio) => {
      ...
      try {
        ...
        if (this._whisperSTT && audio instanceof Blob) {
          const result = await this._whisperSTT.transcribe(audio);
          text = result.text || "";
        } else {
