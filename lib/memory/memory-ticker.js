...
import { createCompileStatePersistence, createCircuitBreaker } from "./compile-retry.js";
...
export function createMemoryTicker(opts) {
  ...
  const statePersistence = createCompileStatePersistence({
    stateDir: memoryDir,
    logger: {
  ...
  const _stepKeys = ["rollingSummary", "compileToday", "compileWeek", "compileLongterm", "compileFacts", "deepMemory", "qualityCheck", "forgettingCheck"];
  const _health = {};
  for (const k of _stepKeys) {
    _health[k] = { lastSuccessAt: null, lastErrorAt: null, lastErrorMsg: null, failCount: 0 };
  }

  const _qualityMonitor = createQualityMonitor(opts.qualityMonitorConfig);
  ...
  async function _doDaily() {
    ...
    try {
      ...
      // Step 0 + Step 1 并行执行（compileToday 和 compileWeek 互不依赖）
      const step0Needed = !_dailyStepsCompleted.has("compileToday");
      const step1Needed = !_dailyStepsCompleted.has("compileWeek");

      if (step0Needed || step1Needed) {
        const jobs = [];
      ...
      // Step 6: quality-check（评估记忆质量，检测低质量记忆）
      if (!_dailyStepsCompleted.has("qualityCheck")) {
        try {
          const allFacts = factStore.getAll();
          if (allFacts.length > 0) {
            const result = _qualityMonitor.processFacts(allFacts);
            const lowQualityCount = result.lowQualityFacts.length;
            if (lowQualityCount > 0) {
              log.log(`quality-check: ${lowQualityCount} low-quality memories detected`);
            }
            const stats = factStore.getQualityStats();
            log.log(`quality-check: average quality ${stats.averageQuality} (${stats.total} memories)`);
          }
          _dailyStepsCompleted.add("qualityCheck");
          _markSuccess("qualityCheck");
          _markStepRecovered("quality-check");
        } catch (err) {
          hasFailed = true;
          _markFailure("qualityCheck", err);
          _logStepError("quality-check", err);
        }
      }

      // Step 7: forgetting-check（评估记忆衰减，将低于阈值的记忆归档并从活跃库移除）
      if (!_dailyStepsCompleted.has("forgettingCheck") && factStore._forgettingCurve) {
        try {
          const evaluation = factStore._forgettingCurve.evaluateForgetting();
          const toArchive = evaluation.toArchive;
          if (toArchive.length > 0 && factStore._archiveManager) {
            factStore._archiveManager.archiveBatch(toArchive, "decay_below_threshold");
            let deletedCount = 0;
            for (const fact of toArchive) {
              if (factStore.delete(fact.id)) deletedCount++;
            }
            log.log(`forgetting-check: ${toArchive.length} memories evaluated, ${deletedCount} archived and removed`);
          }
          _dailyStepsCompleted.add("forgettingCheck");
          _markSuccess("forgettingCheck");
          _markStepRecovered("forgetting-check");
        } catch (err) {
          hasFailed = true;
          _markFailure("forgettingCheck", err);
          _logStepError("forgetting-check", err);
        }
      }
  ...
  async function _tickCore() {
    ...
    if (savedState?.circuitBreakerState) {
      circuitBreaker.importState(savedState.circuitBreakerState);
      log.log(`circuit breaker state restored: ${savedState.circuitBreakerState.state}`);
    }

    const todayStr = getLogicalDay().logicalDate;
    if (savedState?.lastCompileDate === todayStr && savedState?.completedSteps) {
      for (const step of savedState.completedSteps) {
        _dailyStepsCompleted.add(step);
      }
      log.log(`daily steps restored from state: ${[..._dailyStepsCompleted].join(", ")}`);
    }

    await _recoverUnsummarized();
    if (_lastDailyJobDate !== todayStr) {
      await _doDaily();
    }
  ...
  async function _persistCompileState() {
    try {
      await statePersistence.saveState({
        lastCompileDate: getLogicalDay().logicalDate,
        completedSteps: [..._dailyStepsCompleted],
        circuitBreakerState: circuitBreaker.exportState(),
      });
    } catch (err) {
      debugLog()?.error("memory", `failed to persist compile state: ${err.message}`);
    }
  }
  ...
  function getCircuitBreakerStatus() {
    const cbState = circuitBreaker.exportState();
    return {
      state: cbState.state,
  ...
  return { start, stop, tick, triggerNow, notifyTurn, notifySessionEnd, notifyPromoted, flushSession, flushSessionAndCompile, getHealthStatus, getCircuitBreakerStatus };
}
