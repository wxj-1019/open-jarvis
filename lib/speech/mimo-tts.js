...
function getMimoConfig(engine) {
  // 通过 Provider 系统获取凭证
  const credsResult = engine?.providerRegistry?.getCredentials?.("mimo-tts");

  if (!credsResult?.apiKey) {
    throw new Error(
      "MiMo TTS API key not configured. Please add it in Settings → Providers → Xiaomi MiMo TTS."
    );
  }

  const baseUrl = credsResult.baseUrl || DEFAULT_BASE_URL;

  // 从 Provider 系统获取首选模型
  let model = DEFAULT_MODEL;
  try {
    const models = engine?.providerRegistry?.getProviderModels?.("mimo-tts");
    if (models?.length > 0) {
      model = models[0];
    }
  } catch (err) {
    log?.warn?.("Failed to get TTS model from provider config, using default:", model);
  }

  return { apiKey: credsResult.apiKey, baseUrl, model };
}
