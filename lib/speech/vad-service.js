...
export class VADService extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    /** @type {'rms'|'hybrid'|'silero'} VAD 模式 */
    this._mode = opts.mode ?? 'rms';

    /** @type {number[]} 噪声样本缓冲区（自适应阈值用） */
    this._noiseSamples = [];
    /** @type {boolean} 是否启用自适应阈值 */
    this._adaptiveThreshold = opts.adaptiveThreshold ?? false;
    /** @type {number} 阈值倍数 */
    this._thresholdMultiplier = opts.thresholdMultiplier ?? 3;
  ...
  processConfidence(confidence) {
    ...
    const isSpeech = confidence >= (this._silenceThreshold || 0.5);
    if (isSpeech) {
  ...
  /**
   * 启动自适应阈值校准。
   * 采样环境噪声，计算噪声基线，动态调整 silenceThreshold。
   */
  startCalibration() {
    if (!this._adaptiveThreshold) return;
    this._noiseSamples = [];

    this._calibrationTimer = setInterval(() => {
      if (this._noiseSamples.length > 0 && this._state !== VAD_STATE.SPEECH) {
        const sorted = [...this._noiseSamples].sort((a, b) => a - b);
        const p50 = sorted[Math.floor(sorted.length * 0.5)];
        const newThreshold = p50 * this._thresholdMultiplier;
        this._silenceThreshold = Math.max(0.005, Math.min(0.1, newThreshold));
        moduleLog?.info?.(`Adaptive threshold: ${this._silenceThreshold.toFixed(4)}`);
      }
      this._noiseSamples = [];
    }, 30000);
  }
  ...
  _processEnergy(energy) {
    // 收集噪声样本（仅在静音状态时）
    if (this._adaptiveThreshold && this._state !== VAD_STATE.SPEECH) {
      this._noiseSamples.push(energy);
      if (this._noiseSamples.length > 500) {
        this._noiseSamples.shift();
      }
    }
  ...
  destroy() {
    ...
    moduleLog?.info?.("VADService destroyed");
  }
}
