/**
 * config-scope.js + migrate-config-scope.js å•å…ƒæµ‹è¯•
 *
 * æµ‹è¯•ï¼šsplitByScopeã€injectGlobalFieldsã€migrateConfigScope
 */
...
describe("splitByScope", () => {
  ...
  it("desk.home_folder is agent-scoped, heartbeat_interval also agent-scoped", () => {
    ...
    // home_folder is now agent scope â€?NOT extracted as global
...
describe("migrateConfigScope", () => {
  ...
  it("does not overwrite existing non-default preferences (stable is the default so agent beta wins)", () => {
    ...
    // Preferences already has the default "stable" â€?agent value should win
    ...
    // prefs had "nightly" (non-default) â€?must NOT be overwritten by agent "beta"
  ...
  it("is idempotent â€?second run is a no-op", () => {
    ...
    // Second run must be skipped entirely â€?locale stays "en"
