...
export class UserContextTracker {
  ...
  constructor({ eventBus, options }) {
    ...
    // 浏览器上下文（由 BrowserContextAdapter 写入）
  ...
  start() {
    ...
    this._unsubscribers = [
      ...
      this._eventBus.subscribe(
        this._onBrowserContextChanged.bind(this),
        { types: ["browser_context_changed"] },
      ),
    ];
  ...
  /**
   * 处理浏览器上下文变化
   * @param {object} event { type, url, title, searchQuery, selection, article, timestamp }
   */
  _onBrowserContextChanged(event) {
    if (!this._running) return;
    ...
    log.log(`浏览器上下文更新: ${event.title} (${event.url})`);
  }
  ...
  getContextSummary(locale) {
    ...
    if (this._browserContext) {
      ...
      parts.push(
        isZh
          ? `当前浏览: ${bc.title} (${bc.url})`
          : `Currently browsing: ${bc.title} (${bc.url})`,
      );
      if (bc.searchQuery) {
        parts.push(
          isZh
            ? `搜索查询: "${bc.searchQuery}"`
            : `Search query: "${bc.searchQuery}"`,
        );
      }
      if (bc.selection) {
        parts.push(
          isZh
            ? `选中内容: "${bc.selection.slice(0, 100)}"`
            : `Selected text: "${bc.selection.slice(0, 100)}"`,
        );
      }
    }
