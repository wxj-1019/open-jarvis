...
import { buildFts5Query, buildLikePattern, rerankResults } from "./fts5-query-builder.js";
...
// ── 连接池：按 dbPath 复用 better-sqlite3 连接，避免多 Agent 重复打开 ──
const dbPool = new Map(); // dbPath → { db, refCount }

function acquireDb(dbPath, Database) {
  if (dbPool.has(dbPath)) {
    const entry = dbPool.get(dbPath);
    entry.refCount++;
    return entry.db;
  }
  const db = new Database(dbPath);
  dbPool.set(dbPath, { db, refCount: 1 });
  return db;
}

function releaseDb(dbPath) {
  const entry = dbPool.get(dbPath);
  if (!entry) return;
  entry.refCount--;
  if (entry.refCount <= 0) {
...
export class FactStore {
  ...
  constructor(dbPath, opts = {}) {
    ...
    this.db = acquireDb(dbPath, Database);
    this._dbPath = dbPath;  // 用于释放连接池
    ...
    // FTS5 查询增强改为查询时动态构建（不再依赖独立数据库）
    this._fts5QueryEnabled = opts.enableFts5Optimization !== false;
  ...
  _migrateToFactType() {
    ...
    if (!columns.some((col) => col.name === "type")) {
      this.db.exec("ALTER TABLE facts ADD COLUMN type TEXT NOT NULL DEFAULT 'fact'");
    }

    // 根据已有 tags 回填 type：含 "feedback" → 'feedback'，含 "preference" → 'preference'
    this.db.exec(`
      UPDATE facts SET type = 'feedback'
      WHERE tags LIKE '%"feedback"%' AND type = 'fact'
    `);
    this.db.exec(`
      UPDATE facts SET type = 'preference'
      WHERE tags LIKE '%"preference"%' AND type = 'fact'
    `);

    this.db.exec("CREATE INDEX IF NOT EXISTS idx_facts_type ON facts(type)");
    log.log("fact type column added and backfilled");
  }

  computeAndStoreQualityScores(facts) {
    if (!facts || facts.length === 0) return;

    if (!this._stmts._updateQuality) {
      this._stmts._updateQuality = this.db.prepare(`
        UPDATE facts SET 
          quality_specificity = ?,
          quality_recency = ?,
          quality_relevance = ?,
    ...
    const updateStmt = this._stmts._updateQuality;
  ...
  _computeQualityForNewFact(id, factText, tags, createdAt) {
    try {
      ...
      if (!this._stmts._updateQualitySingle) {
        this._stmts._updateQualitySingle = this.db.prepare(`
          UPDATE facts SET 
            quality_specificity = ?,
            quality_recency = ?,
            quality_relevance = ?,
      ...
      this._stmts._updateQualitySingle.run(scores.specificity, scores.recency, scores.relevance, scores.consistency, scores.usage, scores.composite, id);
    } catch (err) {
  ...
  getFactsWithFeedback(type) {
    ...
    return allFacts.filter((f) => {
      const fb = f.user_feedback || {};
      return type === "important" ? fb.important : fb.useless;
    });
  }

  /**
   * 获取嵌入模型实例（供外部模块如 DocStore 复用）
   * @returns {EmbeddingModelManager|null}
   */
  getEmbeddingModel() {
    return this._embeddingModel || null;
  }

  /**
   * 按事实类型检索
   * @param {'fact'|'feedback'|'preference'} type - 事实类型
   * @param {number} [limit=50] - 最大返回数
   * @returns {Array<object>}
   */
  getByType(type, limit = 50) {
    if (!["fact", "feedback", "preference"].includes(type)) return [];
    const rows = this.db.prepare(`
      SELECT * FROM facts
      WHERE type = ?
        AND json_extract(user_feedback, '$.outdated') IS NOT 1
      ORDER BY time DESC
      LIMIT ?
    `).all(type, limit);
    return rows.map((row) => this._rowToFact(row));
  ...
  /**
   * 获取活跃的反馈/偏好事实，供系统提示词注入
   * 排除已标记 outdated 或 useless 的事实
   * @param {number} [limit=20] - 最大返回数
   * @returns {Array<object>}
   */
  getActiveFeedback(limit = 20) {
    const rows = this.db.prepare(`
      SELECT * FROM facts
      WHERE type IN ('feedback', 'preference')
        AND json_extract(user_feedback, '$.outdated') IS NOT 1
        AND json_extract(user_feedback, '$.useless') IS NOT 1
      ORDER BY time DESC
      LIMIT ?
    `).all(limit);
    return rows.map((row) => this._rowToFact(row));
  ...
  _prepareStatements() {
    this._stmts = {
      insert: this.db.prepare(`
        INSERT INTO facts (fact, search_text, tags, type, time, effective_time, session_id, created_at)
        VALUES (@fact, @searchText, @tags, @type, @time, @effectiveTime, @sessionId, @createdAt)
      `),
  ...
  /**
   * 新增一条元事实
   * @param {{ fact: string, tags: string[], type?: string, time?: string, session_id?: string }} entry
   * @returns {{ id: number }}
   */
  add(entry) {
    const { id } = this._addRaw(entry);
    ...
    if (this._vectorEngine && this._embeddingModel?.isAvailable) {
      this._storeEmbeddingAsync(id, cleaned, entry.time).catch(err => {
        log?.warn?.(`Failed to store embedding for fact ${id}: ${err.message}`);
      });
    }
  ...
  /**
   * 仅执行原始插入（无质量评分、无嵌入），供 addBatch 事务内使用
   * @returns {{ id: number }}
   */
  _addRaw(entry) {
    const { cleaned } = scrubPII(entry.fact);
    const factType = (entry.type === "feedback" || entry.type === "preference") ? entry.type : "fact";
    const now = new Date().toISOString();
  ...
  addBatch(entries) {
    ...
    // 逐条嵌入（fire-and-forget，兼容旧行为）
    for (let i = 0; i < entries.length; i++) {
      if (this._vectorEngine && this._embeddingModel?.isAvailable) {
        this._storeEmbeddingAsync(ids[i], entries[i].fact, entries[i].time).catch(err => {
          log?.warn?.(`Failed to store embedding for fact ${ids[i]}: ${err.message}`);
        });
      }
    }
  ...
  /**
   * 批量新增并批量存储嵌入向量（单事务 + 单批嵌入 API 调用）
   * @param {Array<{ fact: string, tags: string[], time?: string, session_id?: string }>} entries
   * @returns {Promise<{ count: number, embedded: number }>}
   */
  async addBatchWithEmbeddings(entries) {
    const ids = [];
    const run = this.db.transaction(() => {
      for (const entry of entries) {
        const { id } = this._addRaw(entry);
    ...
    let embedded = 0;
    if (this._vectorEngine && this._embeddingModel?.isAvailable && ids.length > 0) {
      try {
        const texts = entries.map(e => e.fact);
        const embeddings = await this._embeddingModel.getEmbeddings(texts);
        const items = [];
        for (let i = 0; i < ids.length; i++) {
          if (embeddings[i]) {
            items.push({ factId: ids[i], embedding: embeddings[i], time: entries[i].time });
          }
        }
        if (items.length > 0) {
          this._vectorEngine.storeEmbeddings(items);
          embedded = items.length;
        }
      } catch (err) {
        log?.warn?.(`Batch embedding failed: ${err.message}, falling back to sequential`);
        // 降级到逐条嵌入
        for (let i = 0; i < ids.length; i++) {
          this._storeEmbeddingAsync(ids[i], entries[i].fact, entries[i].time).catch(() => {});
        }
      }
    }

    return { count: entries.length, embedded };
  }
  ...
  enhancedSearchFullText(query, opts = {}) {
    ...
    // 使用查询时动态构建增强 FTS5 查询（不依赖独立优化数据库）
    if (this._fts5QueryEnabled) {
      const ftsQuery = buildFts5Query(query);
      if (ftsQuery) {
        try {
          const rows = this._stmts.ftsSearch.all(ftsQuery, limit * 2);
          const results = rows.map((row) => ({
            ...this._rowToFact(row),
            ftsScore: row.rank ? 1 - (row.rank / rows.length) : 0.5,
          }));
          return rerankResults(results, query, { limit }).slice(0, limit);
        } catch {
          // FTS 语法错误 → LIKE 回退
        }
      }
