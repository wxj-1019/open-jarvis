...
import { ArrowsClockwise, Check, X, ShieldCheck } from '@phosphor-icons/react';
import {
...
function ToolModelTestBtn({ modelRef }: { modelRef: unknown }) {
  ...
  return (
    <button className={`${styles['pv-tool-test-btn']} ${styles[status] || ''}`} onClick={test} disabled={status === 'testing'}>
      {status === 'testing' ? (
        <PhosphorIcon icon={ArrowsClockwise} size={12} className={styles['spinning']} />
      ) : status === 'ok' ? (
        <PhosphorIcon icon={Check} size={12} />
      ) : status === 'fail' ? (
        <PhosphorIcon icon={X} size={12} />
      ) : (
        <PhosphorIcon icon={ShieldCheck} size={12} />
      )}
    </button>
  );
}
