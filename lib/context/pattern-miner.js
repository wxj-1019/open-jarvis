...
export class PatternMiner {
  ...
  findFrequentPatterns(events, minSupport = 2) {
    ...
    const sorted = [...events].sort((a, b) => a.timestamp - b.timestamp);
    const limitedEvents = sorted.length > MAX_EVENTS 
      ? sorted.slice(sorted.length - MAX_EVENTS) // 取最近的 MAX_EVENTS 条
      : sorted;
    
    const stateSequence = limitedEvents.map((e) => this.encodeState(e));
