...
function computeChecksumStream(filePath) {
...
export async function exportAgent(agentDir, outputPath) {
  ...
  try {
    ...
    manifest.checksum = await computeChecksumStream(outputPath);
...
export async function importAgent(zipPath, targetDir) {
  ...
  try {
    ...
    if (manifest.checksum) {
      const actual = await computeChecksumStream(zipPath);
    ...
    try {
      ...
      if (err.code === "EXDEV") {
        copyRecursive(tmpDir, targetDir);
