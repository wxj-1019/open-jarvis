...
export class EmbeddingModelManager {
  ...
  async initialize() {
    ...
    if (!this._forceRemote) {
      try {
        ...
        this.log?.info?.("Local embedding model loaded");
        return;
      } catch (err) {
        this.log?.warn?.(`Local model failed to load: ${err.message}`);
      }
    }
    ...
    if (this._remoteApiUrl) {
      ...
      this._initialized = true;
      this.log?.info?.("Using remote embedding API");
      return;
    }

    this.log?.warn?.("No embedding model available");
  }
  ...
  async getEmbedding(text) {
    ...
    try {
      ...
      this.log?.warn?.(`Embedding failed: ${err.message}`);
      return null;
    }
  }
  ...
  async getEmbeddings(texts) {
    ...
    try {
      ...
      this.log?.warn?.(`Batch embedding failed: ${err.message}`);
      return texts.map(() => null);
    }
  }
