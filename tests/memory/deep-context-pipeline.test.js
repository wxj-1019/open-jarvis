...
describe("DeepContextPipeline", () => {
  ...
  describe("start / stop", () => {
    it("start �?_running �?true", () => {
      pipeline.start();
      expect(pipeline._running).toBe(true);
    });

    it("stop �?_running �?false", async () => {
      pipeline.start();
      await pipeline.stop();
  ...
  describe("L1 窗口焦点", () => {
    ...
    it("getRichContext 在收到事件后返回完整上下文结�?, () => {
      pipeline.start();
      bus.emit({
        type: "window_focus_changed",
        app: "Code.exe",
  ...
  describe("事件驱动 rich_context_changed", () => {
    it("L1 变化时发�?rich_context_changed 事件", () => {
      pipeline.start();
      const handler = vi.fn();
