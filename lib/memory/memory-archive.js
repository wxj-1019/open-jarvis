...
export class MemoryArchiveManager {
  ...
  _prepareStatements() {
    this._stmts = {
      insert: this.db.prepare(`
        INSERT OR IGNORE INTO archived_facts (original_id, fact, tags, time, session_id, created_at, archived_at, reason, hit_count, importance)
      ...
      cleanupOld: this.db.prepare(`
        DELETE FROM archived_facts WHERE archived_at < ?
      `),
    };
  }
  ...
  archiveFact(fact, reason = "decay_below_threshold") {
    ...
    this.log?.info?.(`Archived fact ${fact.id}: ${String(fact.fact).slice(0, 50)}...`);
  }
  ...
  cleanupOldArchives(daysOld) {
    ...
    this._stmts.cleanupOld.run(cutoffDate);
