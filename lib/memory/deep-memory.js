 ...
 * @returns {string[]} 自动追加的标签
 */
function classifyFactTags(fact, existingTags) {
  const newTags = [];

  // 检测 feedback 类型：包含纠正信号
  const isFeedback = FEEDBACK_PATTERNS.some((p) => p.test(fact));
  if (isFeedback && !existingTags.includes("feedback")) {
    newTags.push("feedback");
  }

  // 检测 preference 类型：LLM 标签已包含偏好相关标签
  const hasPreferenceTag = existingTags.some((t) => PREFERENCE_LABELS.has(t));
  if (hasPreferenceTag && !existingTags.includes("preference")) {
    newTags.push("preference");
  }
...
export async function processDirtySessions(summaryManager, factStore, resolvedModel, opts = {}) {
  ...
  const processOne = async (session) => {
    try {
      ...
      if (facts.length > 0) {
        // 自动分类：为事实追加 feedback / preference 标签
        const classifiedFacts = facts.map((f) => {
          const autoTags = classifyFactTags(f.fact, f.tags || []);
          return {
            fact: f.fact,
            tags: autoTags.length > 0 ? [...(f.tags || []), ...autoTags] : (f.tags || []),
            time: f.time || null,
