...
export class BrowserContextAdapter extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    this._processingFile = join(this._fallbackDir, "messages.processing.jsonl");
    this._pollIntervalMs = options.pollIntervalMs ?? 1000;
    this._timer = null;
    this._running = false;
    this._isProcessing = false; // 防止并发处理
  }
   ...
   * @private
   */
  _cleanupProcessingFile() {
    try {
      if (existsSync(this._processingFile)) {
        writeFileSync(this._processingFile, ""); // 清空文件内容
      }
    } finally {
      this._isProcessing = false;
    }
