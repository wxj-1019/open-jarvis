...
import { WindowContentExtractor } from "./window-content-extractor.js";
import { WindowsUiaAdapter } from "./adapters/windows-uia-adapter.js";
import { MacosAxAdapter } from "./adapters/macos-ax-adapter.js";
import { LinuxAtspiAdapter } from "./adapters/linux-atspi-adapter.js";
...
export class DeepContextPipeline {
  ...
  constructor({ eventBus, options }) {
    ...
    // Accessibility Tree 提取器（Phase 2）
    this._contentExtractor = new WindowContentExtractor({
  ...
  async _triggerL2(app, title) {
    ...
    // 新增：Accessibility Tree 提取（Phase 2）
    try {
      const a11yContent = await this._contentExtractor.extract({ app, title });
      if (a11yContent.elements.length > 0) {
        if (!this._l2) {
          this._l2 = { sourceType: "a11y", type: "a11y" };
        }
        this._l2.a11yContent = {
          elements: a11yContent.elements.slice(0, 50), // 限制数量
          focusedElement: a11yContent.focusedElement,
          browserUrl: a11yContent.browserUrl,
          source: a11yContent._source,
          strategy: a11yContent._strategy,
        };
        this._updateRichContext();
        log.log("L2 a11y 提取完成: %d elements, source=%s", a11yContent.elements.length, a11yContent._source);
      }
    } catch (err) {
      log.warn("L2 a11y 提取失败: %s", err.message);
    }
  }
