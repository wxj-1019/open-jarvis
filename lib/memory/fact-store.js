...
export class FactStore {
   ...
   * @param {{ Database?: import("better-sqlite3"), vectorDbPath?: string, embeddingModel?: EmbeddingModelManager }} [opts]
   */
  constructor(dbPath, opts = {}) {
    ...
    if (opts.vectorDbPath) {
      try {
        ...
        this.log?.info?.("Vector search engine initialized");
      } catch (err) {
        this.log?.warn?.(`Vector search unavailable: ${err.message}`);
      }
    }
  }
   ...
   */
  async searchWithVectors(query, limit = 20, dateRange) {
    if (!query || !query.trim()) return [];
    ...
    try {
      ...
      const hybridResults = this._vectorEngine.hybridSearch(
        embedding,
        ftsResults.map((r) => ({ id: r.id, rank: ftsResults.indexOf(r) + 1 })),
        limit,
        { dateRange },
      );

      const factMap = new Map();
      for (const fact of [...ftsResults, ...this.getAll()]) {
      ...
      this.log?.warn?.(`Vector search failed, falling back to FTS: ${err.message}`);
      return ftsResults.slice(0, limit).map((r) => ({
        ...r,
        hybridScore: 0.4,
      }));
  ...
  async _storeEmbeddingAsync(factId, fact, time) {
    try {
      ...
      this.log?.warn?.(`Failed to store embedding for fact ${factId}: ${err.message}`);
    }
  }
