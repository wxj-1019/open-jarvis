/**
 * parallel-executor.test.js — DAG 并行步骤执行器测试
 *
 * 覆盖: executeParallel 核心逻辑、concurrency 限制、部分失败、
 *       AbortSignal 取消、PlanExecutor 集成、向后兼容
 */
...
describe("executeParallel", () => {
  ...
  it("并行模式 (concurrency=3) 三个步骤同时执行", async () => {
    ...
    expect(fn).toHaveBeenCalledTimes(3);
    // 并发执行总时间应小于 3*10=30ms（如果串行则需要 30ms+）
  ...
  it("concurrency 限制正确生效（3个步骤 concurrency=2 分两批）", async () => {
    ...
    expect(inFlight.max).toBe(2); // 最多同时 2 个在执行
    for (const s of steps) {
      expect(s.status).toBe(StepStatus.COMPLETED);
    }
  });

  it("部分失败时其余继续执行", async () => {
    const fn = vi.fn(async (step) => {
  ...
  it("onProgress 回调每个步骤完成后触发", async () => {
    const progresses = [];
  ...
  it("AbortSignal 取消剩余批次", async () => {
    ...
    // concurrency=1 分批执行：第一批完成后 signal 已 abort
    await executeParallel(steps, fn, {
      concurrency: 1,
      plan,
    ...
    // s0 已执行，s1/s2 因并发限制在同一批也会执行
    // 但第二批会检查 signal.aborted 并跳过
    expect(fn).toHaveBeenCalledTimes(1); // 仅第一批(s0)执行
  });

  it("并发限制为 0 时自动提升到 1", async () => {
    const fn = vi.fn(async (step) => `result: ${step.id}`);
...
describe("PlanExecutor 集成并行执行", () => {
  ...
  it("多步无依赖 DAG 并行执行 (maxParallel=3)", async () => {
  ...
  it("execute() 参数 maxParallel 覆盖构造函数默认值", async () => {
    mockExecuteIsolated.mockResolvedValueOnce({
      replyText: JSON.stringify({
        steps: [
    ...
    executor = createExecutor({ maxParallel: 1 }); // 构造函数默认 1
    const plan = await executor.execute("Test", { maxParallel: 5 });

    expect(plan.status).toBe(PlanExecutor.PlanStatus?.COMPLETED || "completed");
    // maxParallel 被 params 覆盖为 5（不影响单步执行结果）
  });
});

// ============================================================================
// 3. executeParallel 无 PlanExecutor 上下文的独立测试
// ============================================================================
...
describe("executeParallel 独立边界测试", () => {
  it("DAG 拓扑分批发：依赖满足后逐步执行", async () => {
    ...
    expect(plan.steps[2].status).toBe(StepStatus.PENDING); // c 未执行
