 ...
 * 避免 fake timers �?async setInterval 的兼容性问题�?
 *
...
describe("OSEventSource", () => {
  ...
  describe("start / stop", () => {
    it("启动�?_running �?true 且创�?file watcher �?polling timer", async () => {
    ...
    it("stop �?_running �?false 且清理所有资�?, async () => {
      mockActiveWindow.mockResolvedValue(null);
      await source.start();
      await source.stop();
    ...
    it("�?workspaces 时不创建 file watcher", async () => {
  ...
  describe("window_focus_changed", () => {
    ...
    it("app name �?undefined �?fallback �?unknown", async () => {
      const events = [];
      bus.subscribe((evt) => events.push(evt), { types: ["window_focus_changed"] });
  ...
  describe("updateWorkspaces", () => {
    it("更新 workspaces 后源�?mappings 反映变化", () => {
