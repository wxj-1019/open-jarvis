...
function getMimoConfig(engine) {
  // 通过 Provider 系统获取凭证
  const credentials = engine?.getProviderCredentials?.("mimo-tts");
  ...
  try {
    ...
    if (config?.providers?.["mimo-tts"]?.models?.length > 0) {
      const firstModel = config.providers["mimo-tts"].models[0];
      if (typeof firstModel === "string") {
        model = firstModel;
      } else if (firstModel?.id) {
        model = firstModel.id;
      }
    }
  } catch (err) {
    log?.warn?.("Failed to get TTS model from config, using default:", model);
  }
