...
function spawnViaCmd(command, cwd, { env, onData, signal, timeout }) {
  return spawnAndStream(process.env.COMSPEC || "cmd.exe", cmdArgsForCommand(command), {
    cwd,
    env,
    onData,
    signal,
...
/**
 * 检查可执行文件是否是 GUI 程序
 * @param {string} executable - 可执行文件路径或名称
...
/**
 * 检查可执行文件是否在 GUI 白名单中
 * @param {string} exeName - 可执行文件名称（小写）
 * @param {string[]} guiWhitelist - 白名单列表
 * @returns {boolean}
 */
...
async function spawnViaSandboxHelper({ sandbox, executable, args, cwd, env, onData, signal, timeout, engine }) {
  ...
  const guiWhitelist = sandbox.policy?.guiWhitelist || [];

  // 检查是否是 GUI 程序但不在白名单中
  const exeName = executable.replace(/.*[\\/]/, '').toLowerCase();
  
  if (isGuiApplication(executable, args) && !isExecutableInGuiWhitelist(exeName, guiWhitelist)) {
    // 如果 engine 可用，请求用户批准
    if (engine?.handleGuiWhitelistRequest) {
      const response = await engine.handleGuiWhitelistRequest({
        executable: exeName,
        currentWhitelist: guiWhitelist,
        sessionPath: sandbox.sessionPath || null,
      });

      if (response.approved) {
        // 用户已批准，添加到白名单后继续
        guiWhitelist.push(exeName);
      } else {
        // 用户拒绝，抛出错误
        const error = new Error(
          `[win32-sandbox] GUI application "${exeName}" was denied by user.`
        );
        error.code = 'GUI_WHITELIST_DENIED';
        throw error;
      }
    } else {
      // engine 不可用，抛出原始错误
      const error = new Error(
        `[win32-sandbox] GUI application "${exeName}" is not in the whitelist. ` +
        `Please add it to the GUI whitelist in lib/sandbox/policy.js or approve it via user prompt.`
      );
      error.code = 'GUI_WHITELIST_REQUIRED';
      error.executable = exeName;
      error.guiWhitelist = guiWhitelist;
      throw error;
    }
  }
 ...
 */
export function createWin32Exec({ sandbox = null, engine = null } = {}) {
