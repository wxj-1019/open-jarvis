...
export class ForgettingCurveEngine {
  ...
  /**
   * Calculate retention for a single fact (used by search ranking).
   * Returns a value in [0, 1] representing how "fresh" the memory is.
   * @param {{ created_at: string, importance?: number, hit_count?: number }} fact
   * @returns {number}
   */
  calculateFactRetention(fact) {
    if (!fact?.created_at) return 1;
    const ageDays = this._ageInDays(fact.created_at);
    return this.calculateRetentionWithReinforcement(
      ageDays,
      fact.importance ?? 0,
      fact.hit_count ?? 0,
    );
