 ...
 * @returns {boolean}
 */
function isExecutableInGuiWhitelist(executable, guiWhitelist) {
...
function spawnViaSandboxHelper({ sandbox, executable, args, cwd, env, onData, signal, timeout }) {
  ...
  // 检查是否是 GUI 程序但不在白名单中
  const isGuiApp = executable.toLowerCase().includes('powershell') || 
                   executable.toLowerCase().includes('cmd.exe') ||
                   executable.toLowerCase().includes('msg.exe') ||
                   args.some(arg => arg.toLowerCase().includes('-window') || arg.toLowerCase().includes('-gui'));
  
  if (isGuiApp && !isExecutableInGuiWhitelist(executable, guiWhitelist)) {
    const exeName = executable.replace(/.*[\\/]/, '');
